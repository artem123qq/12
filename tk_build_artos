...
class GUIBuilderApp:
    def __init__(self, root):
        ...
        self.drag_data        = {"items":[], "start_x":0, "start_y":0,
                                 ...
                                 "marquee_selecting":False, "marquee_rect":None}
        self.nav_panel = None
        self.clipboard        = None
        ...
        # API ключи удалены (ИИ-интеграция отключена)
        ...
        self.setup_ui()
        self.bind_keys()
        # self.load_templates()  # ← УДАЛИЛ: шаблоны теперь создаются внутри setup_ui или удалены
    ...
    def display_current_screen(self):
        """Отображает текущий экран на холсте - ПОЛНОСТЬЮ УНИЧТОЖАЕТ старые виджеты"""
        # 1. Полная очистка холста
        self.canvas.delete("all")

        # 2. ВАЖНО: Уничтожаем реальные Tk-фреймы, чтобы не было "призраков" и ошибок active
        for wo in self.widgets:
            if hasattr(wo, 'frame') and wo.frame and wo.frame.winfo_exists():
                wo.frame.destroy()
            wo.widget_instance = None
            wo.frame = None
            wo.selection_rect = None
            wo.resize_handles = []
        ...
        # 3. Создаем виджеты текущего экрана
        for wo in self.screens[self.current_screen]:
            ...
            nw.visible = wo.visible
            nw.locked = wo.locked
            nw.group_id = getattr(wo, 'group_id', None)
            self.widgets.append(nw)
            self.render_widget(nw)
    ...
    def bind_navigation_events(self):
        """Привязывает события для навигации между экранами (без дублирования)"""
        import re
        for wo in self.widgets:
            if not wo.events or not wo.frame or not wo.frame.winfo_exists():
                continue

            for event_cmd, command in wo.events.items():
                if command.startswith("switch_screen"):
                    ...
                    try:
                        wo.frame.unbind(event_cmd)
                    except:
                        pass
                    for child in wo.frame.winfo_children():
                        try:
                            child.unbind(event_cmd)
                        except:
                            pass

                    # Создаем новую привязку
                    def make_switch(sn=screen_name):
                        return lambda e: self.switch_to_screen_direct(sn)

                    wo.frame.bind(event_cmd, make_switch)
                    for child in wo.frame.winfo_children():
                        child.bind(event_cmd, make_switch)
    ...
    def load_templates(self):
        pass  # Шаблоны удалены

    def insert_template(self, event):
        messagebox.showinfo("Информация", "Шаблоны форм удалены из интерфейса.")
    def setup_ui(self):
        # Top toolbar
        ...
        ttk.Separator(top, orient="vertical").pack(side=tk.LEFT, padx=6, fill=tk.Y)
        ttk.Button(top, text="⬜ Выделить область", command=self.set_marquee_mode).pack(side=tk.LEFT, padx=2)
        ...
        ttk.Label(self.toolbar_inner, text="📦 Раздел шаблонов удалён",
                  foreground="gray", font=("Arial", 8)).pack(pady=10, padx=4)
        ...
        self.ctx.add_command(label="📦 Группировать (Ctrl+G)", command=self.group_selected)
        self.ctx.add_command(label="📤 Разгруппировать (Ctrl+Shift+G)", command=self.ungroup_selected)
        self.ctx.add_command(label="👁 Выделить группу", command=self._select_current_group)
        self.ctx.add_separator()
    ...
    def update_property_panel(self):
        """Обновляет панель свойств с персональными параметрами для каждого типа виджета"""
        ...
        # ========== ПОЗИЦИЯ ==========
        pos_frame = ttk.LabelFrame(self.prop_inner, text="📐 Позиция", padding=5)
        pos_frame.pack(fill=tk.X, pady=5, padx=5)
        self._add_prop_row(pos_frame, "x", "X:", wo.x, "int")
        self._add_prop_row(pos_frame, "y", "Y:", wo.y, "int")

        # ========== РАЗМЕР ==========
        size_frame = ttk.LabelFrame(self.prop_inner, text="📏 Размер", padding=5)
        size_frame.pack(fill=tk.X, pady=5, padx=5)
        self._add_prop_row(size_frame, "width", "Ширина:", wo.current_width, "int")
        self._add_prop_row(size_frame, "height", "Высота:", wo.current_height, "int")

        # ========== ПАРАМЕТРЫ СПЕЦИФИЧНЫЕ ДЛЯ ТИПА ==========
        self._add_widget_specific_props(wt, cfg)

        # Обновить позицию скролла
        self.prop_inner.update_idletasks()

    def _add_widget_specific_props(self, wt, cfg):
        """Добавляет параметры специфичные для типа виджета"""

        # ========== Label ==========
        if wt == "Label":
            frame = ttk.LabelFrame(self.prop_inner, text="📝 Параметры Label", padding=5)
            frame.pack(fill=tk.X, pady=5, padx=5)
            self._add_prop_row(frame, "text", "Текст:", cfg.get("text", ""), "str")
            self._add_color_row(frame, "bg", "Фон:", cfg.get("bg", "#f0f0f0"))
            self._add_color_row(frame, "fg", "Цвет текста:", cfg.get("fg", "black"))
            self._add_prop_row(frame, "font", "Шрифт:", cfg.get("font", "Arial 10"), "str")
            self._add_checkbox_row(frame, "wraplength", "Перенос текста", cfg.get("wraplength", 0) > 0)
            self._add_prop_row(frame, "anchor", "Выравнивание:", cfg.get("anchor", "center"), "str")
            self._add_prop_row(frame, "justify", "Justify:", cfg.get("justify", "center"), "str")

        # ========== Button ==========
        elif wt == "Button":
            frame = ttk.LabelFrame(self.prop_inner, text="🔘 Параметры Button", padding=5)
            frame.pack(fill=tk.X, pady=5, padx=5)
            self._add_prop_row(frame, "text", "Текст:", cfg.get("text", ""), "str")
            self._add_color_row(frame, "bg", "Фон:", cfg.get("bg", "#e0e0e0"))
            self._add_color_row(frame, "fg", "Цвет текста:", cfg.get("fg", "black"))
            self._add_prop_row(frame, "font", "Шрифт:", cfg.get("font", "Arial 10"), "str")
            self._add_prop_row(frame, "relief", "Рамка:", cfg.get("relief", "raised"), "str")
            self._add_prop_row(frame, "borderwidth", "Ширина рамки:", cfg.get("borderwidth", 2), "int")
            self._add_checkbox_row(frame, "state", "Активна", cfg.get("state", "normal") == "normal")

        # ========== Entry ==========
        elif wt == "Entry":
            frame = ttk.LabelFrame(self.prop_inner, text="📝 Параметры Entry", padding=5)
            frame.pack(fill=tk.X, pady=5, padx=5)
            self._add_prop_row(frame, "text", "Текст:", cfg.get("text", ""), "str")
            self._add_color_row(frame, "bg", "Фон:", cfg.get("bg", "white"))
            self._add_color_row(frame, "fg", "Цвет текста:", cfg.get("fg", "black"))
            self._add_prop_row(frame, "font", "Шрифт:", cfg.get("font", "Arial 10"), "str")
            self._add_prop_row(frame, "width", "Ширина (симв.):", cfg.get("width", 20), "int")
            self._add_checkbox_row(frame, "show", "Скрыть ввод (пароль)", cfg.get("show", "") == "*")
            self._add_checkbox_row(frame, "state", "Активно", cfg.get("state", "normal") == "normal")

        # ========== Checkbutton ==========
        elif wt == "Checkbutton":
            frame = ttk.LabelFrame(self.prop_inner, text="☑️ Параметры Checkbutton", padding=5)
            frame.pack(fill=tk.X, pady=5, padx=5)
            self._add_prop_row(frame, "text", "Текст:", cfg.get("text", ""), "str")
            self._add_color_row(frame, "bg", "Фон:", cfg.get("bg", "#f0f0f0"))
            self._add_color_row(frame, "fg", "Цвет текста:", cfg.get("fg", "black"))
            self._add_prop_row(frame, "font", "Шрифт:", cfg.get("font", "Arial 10"), "str")
            self._add_checkbox_row(frame, "variable", "Выбрано", cfg.get("variable", 1) == 1)

        # ========== Radiobutton ==========
        elif wt == "Radiobutton":
            frame = ttk.LabelFrame(self.prop_inner, text="🔘 Параметры Radiobutton", padding=5)
            frame.pack(fill=tk.X, pady=5, padx=5)
            self._add_prop_row(frame, "text", "Текст:", cfg.get("text", ""), "str")
            self._add_color_row(frame, "bg", "Фон:", cfg.get("bg", "#f0f0f0"))
            self._add_color_row(frame, "fg", "Цвет текста:", cfg.get("fg", "black"))
            self._add_prop_row(frame, "font", "Шрифт:", cfg.get("font", "Arial 10"), "str")
            self._add_prop_row(frame, "value", "Значение:", cfg.get("value", ""), "str")

        # ========== Listbox ==========
        elif wt == "Listbox":
            frame = ttk.LabelFrame(self.prop_inner, text="📋 Параметры Listbox", padding=5)
            frame.pack(fill=tk.X, pady=5, padx=5)
            self._add_color_row(frame, "bg", "Фон:", cfg.get("bg", "white"))
            self._add_color_row(frame, "fg", "Цвет текста:", cfg.get("fg", "black"))
            self._add_prop_row(frame, "font", "Шрифт:", cfg.get("font", "Arial 10"), "str")
            self._add_prop_row(frame, "width", "Ширина (симв.):", cfg.get("width", 20), "int")
            self._add_prop_row(frame, "height", "Высота (строк):", cfg.get("height", 10), "int")
            self._add_checkbox_row(frame, "selectmode", "Множественный выбор", cfg.get("selectmode", "single") == "multiple")
            self._add_prop_row(frame, "items", "Элементы (через запятую):", cfg.get("items", ""), "str")

        # ========== Text ==========
        elif wt == "Text":
            frame = ttk.LabelFrame(self.prop_inner, text="📄 Параметры Text", padding=5)
            frame.pack(fill=tk.X, pady=5, padx=5)
            self._add_color_row(frame, "bg", "Фон:", cfg.get("bg", "white"))
            self._add_color_row(frame, "fg", "Цвет текста:", cfg.get("fg", "black"))
            self._add_prop_row(frame, "font", "Шрифт:", cfg.get("font", "Arial 10"), "str")
            self._add_prop_row(frame, "width", "Ширина (симв.):", cfg.get("width", 40), "int")
            self._add_prop_row(frame, "height", "Высота (строк):", cfg.get("height", 10), "int")
            self._add_prop_row(frame, "wrap", "Перенос:", cfg.get("wrap", "word"), "str")
            self._add_checkbox_row(frame, "state", "Редактируемо", cfg.get("state", "normal") == "normal")

        # ========== Scale ==========
        elif wt == "Scale":
            frame = ttk.LabelFrame(self.prop_inner, text="🎚️ Параметры Scale", padding=5)
            frame.pack(fill=tk.X, pady=5, padx=5)
            self._add_prop_row(frame, "from_", "Минимум:", cfg.get("from_", 0), "int")
            self._add_prop_row(frame, "to", "Максимум:", cfg.get("to", 100), "int")
            self._add_prop_row(frame, "resolution", "Шаг:", cfg.get("resolution", 1), "float")
            self._add_prop_row(frame, "orient", "Ориентация:", cfg.get("orient", "horizontal"), "str")
            self._add_prop_row(frame, "length", "Длина:", cfg.get("length", 100), "int")
            self._add_checkbox_row(frame, "showvalue", "Показывать значение", cfg.get("showvalue", True))
            self._add_prop_row(frame, "tickinterval", "Интервал меток:", cfg.get("tickinterval", 0), "int")

        # ========== Spinbox ==========
        elif wt == "Spinbox":
            frame = ttk.LabelFrame(self.prop_inner, text="🔢 Параметры Spinbox", padding=5)
            frame.pack(fill=tk.X, pady=5, padx=5)
            self._add_prop_row(frame, "from_", "Минимум:", cfg.get("from_", 0), "int")
            self._add_prop_row(frame, "to", "Максимум:", cfg.get("to", 100), "int")
            self._add_prop_row(frame, "increment", "Шаг:", cfg.get("increment", 1), "int")
            self._add_prop_row(frame, "width", "Ширина (симв.):", cfg.get("width", 10), "int")
            self._add_prop_row(frame, "font", "Шрифт:", cfg.get("font", "Arial 10"), "str")

        # ========== Combobox ==========
        elif wt == "Combobox":
            frame = ttk.LabelFrame(self.prop_inner, text="📦 Параметры Combobox", padding=5)
            frame.pack(fill=tk.X, pady=5, padx=5)
            self._add_prop_row(frame, "values", "Значения (через запятую):", cfg.get("values", ""), "str")
            self._add_prop_row(frame, "width", "Ширина (симв.):", cfg.get("width", 20), "int")
            self._add_prop_row(frame, "font", "Шрифт:", cfg.get("font", "Arial 10"), "str")
            self._add_checkbox_row(frame, "state", "Редактируемо", cfg.get("state", "normal") == "normal")
        ...
        # ========== Progressbar ==========
            ...
            frame = ttk.LabelFrame(self.prop_inner, text="📊 Параметры Progressbar", padding=5)
            frame.pack(fill=tk.X, pady=5, padx=5)
            self._add_prop_row(frame, "value", "Значение:", cfg.get("value", 0), "int")
            self._add_prop_row(frame, "maximum", "Максимум:", cfg.get("maximum", 100), "int")
            self._add_prop_row(frame, "length", "Длина:", cfg.get("length", 100), "int")
            self._add_prop_row(frame, "mode", "Режим:", cfg.get("mode", "determinate"), "str")
            self._add_prop_row(frame, "orient", "Ориентация:", cfg.get("orient", "horizontal"), "str")

        # ========== Frame ==========
        elif wt == "Frame":
            frame = ttk.LabelFrame(self.prop_inner, text="🖼️ Параметры Frame", padding=5)
            frame.pack(fill=tk.X, pady=5, padx=5)
            self._add_color_row(frame, "bg", "Фон:", cfg.get("bg", "#e8e8e8"))
            self._add_prop_row(frame, "relief", "Рамка:", cfg.get("relief", "flat"), "str")
            self._add_prop_row(frame, "borderwidth", "Ширина рамки:", cfg.get("borderwidth", 0), "int")

        # ========== Canvas ==========
        elif wt == "Canvas":
            frame = ttk.LabelFrame(self.prop_inner, text="🎨 Параметры Canvas", padding=5)
            frame.pack(fill=tk.X, pady=5, padx=5)
            self._add_color_row(frame, "bg", "Фон:", cfg.get("bg", "white"))
            self._add_prop_row(frame, "relief", "Рамка:", cfg.get("relief", "flat"), "str")
            self._add_prop_row(frame, "borderwidth", "Ширина рамки:", cfg.get("borderwidth", 0), "int")
        ...
        # ========== Notebook ==========
            ...
            frame = ttk.LabelFrame(self.prop_inner, text="📑 Параметры Notebook", padding=5)
            frame.pack(fill=tk.X, pady=5, padx=5)
            self._add_prop_row(frame, "tabs", "Вкладки (через запятую):", cfg.get("tabs", "Tab 1,Tab 2"), "str")

        # ========== Separator ==========
        elif wt == "Separator":
            frame = ttk.LabelFrame(self.prop_inner, text="➖ Параметры Separator", padding=5)
            frame.pack(fill=tk.X, pady=5, padx=5)
            self._add_prop_row(frame, "orient", "Ориентация:", cfg.get("orient", "horizontal"), "str")

        # ========== Scrollbar ==========
        elif wt == "Scrollbar":
            frame = ttk.LabelFrame(self.prop_inner, text="↕️ Параметры Scrollbar", padding=5)
            frame.pack(fill=tk.X, pady=5, padx=5)
            self._add_prop_row(frame, "orient", "Ориентация:", cfg.get("orient", "vertical"), "str")
            self._add_prop_row(frame, "width", "Ширина:", cfg.get("width", 16), "int")

        # ========== Menubutton ==========
        elif wt == "Menubutton":
            frame = ttk.LabelFrame(self.prop_inner, text="📂 Параметры Menubutton", padding=5)
            frame.pack(fill=tk.X, pady=5, padx=5)
            self._add_prop_row(frame, "text", "Текст:", cfg.get("text", ""), "str")
            self._add_color_row(frame, "bg", "Фон:", cfg.get("bg", "#e0e0e0"))
            self._add_color_row(frame, "fg", "Цвет текста:", cfg.get("fg", "black"))
            self._add_prop_row(frame, "font", "Шрифт:", cfg.get("font", "Arial 10"), "str")
            self._add_prop_row(frame, "menu_items", "Пункты меню (через \n):", cfg.get("menu_items", ""), "str")

        # ========== PanedWindow ==========
        elif wt == "PanedWindow":
            frame = ttk.LabelFrame(self.prop_inner, text="⚖️ Параметры PanedWindow", padding=5)
            frame.pack(fill=tk.X, pady=5, padx=5)
            self._add_prop_row(frame, "orient", "Ориентация:", cfg.get("orient", "horizontal"), "str")
            self._add_prop_row(frame, "sashwidth", "Ширина разделителя:", cfg.get("sashwidth", 3), "int")
            self._add_prop_row(frame, "sashrelief", "Стиль разделителя:", cfg.get("sashrelief", "flat"), "str")
    ...
    def bind_keys(self):
        ...
        r.bind("<Control-Shift-G>", lambda e: self.ungroup_selected())
        r.bind("<m>",               lambda e: self.set_marquee_mode())
    ...
    def _esc(self):
        # Отменяем выделение прямоугольником если активно
        if self.drag_data.get("marquee_selecting"):
            self.drag_data["marquee_selecting"] = False
            if self.drag_data.get("marquee_rect"):
                self.canvas.delete(self.drag_data["marquee_rect"])
                self.drag_data["marquee_rect"] = None
        self.current_mode = "move"
        self.mode_label.config(text="Режим: Перемещение")
        self.canvas.config(cursor="")
    ...
    def set_marquee_mode(self):
        """Включает режим выделения прямоугольником (как в Photoshop)"""
        self.current_mode = "marquee"
        self.mode_label.config(text="Режим: Выделение области (тяните мышью для выделения)")
        self.deselect_all()
        self.canvas.config(cursor="crosshair")

    # ── CANVAS EVENTS ─────────────────────────────────────────────────────────
    def on_canvas_press(self, event):
        """Обработка клика по холсту — ИСПРАВЛЕННЫЙ МУЛЬТИ-ВЫБОР"""

        # === Режимы добавления/панорамы/рамки ===
        ...
        if self.current_mode == "pan":
            ...
            return
        if self.current_mode == "marquee":
            self.deselect_all()
            self.drag_data["marquee_selecting"] = True
            self.drag_data["marquee_start_x"] = event.x
            self.drag_data["marquee_start_y"] = event.y
            self.drag_data["marquee_rect"] = self.canvas.create_rectangle(
                event.x, event.y, event.x, event.y,
                outline="#1a6bff", width=1, dash=(4, 4),
                fill="#1a6bff", stipple="gray50", tags="marquee")
            return

        # === Перетаскивание направляющих ===
        for g in self.guides:
            if g["type"] == "v" and abs(event.x - g["pos"]) <= 5:
                self.dragging_guide = g;
                return
            if g["type"] == "h" and abs(event.y - g["pos"]) <= 5:
                self.dragging_guide = g;
                return

        # === Хват за ручку изменения размера ===
        for wo in self.widgets:
        ...
        if clicked:
            ...
            # 🎯 МУЛЬТИ-ВЫБОР (Ctrl/Shift + клик)
            if ctrl_pressed or shift_pressed:
                # Если виджет в группе — работаем со ВСЕЙ группой
                if clicked.group_id:
                    group_ids = [str(w.id) for w in self.widgets if w.group_id == clicked.group_id]
                    # Проверяем: если хотя бы один из группы уже выделен — убираем ВСЮ группу
                    if any(gid in self.selected_widgets for gid in group_ids):
                        for gid in group_ids:
                            if gid in self.selected_widgets:
                                self.selected_widgets.remove(gid)
                    else:
                        # Иначе добавляем ВСЮ группу
                        for gid in group_ids:
                            if gid not in self.selected_widgets:
                                self.selected_widgets.append(gid)
                else:
                    # Обычный виджет — просто переключатель
                    wid = str(clicked.id)
                ...
                # 🔄 ПРИМЕНЯЕМ ВЫДЕЛЕНИЕ (без сброса drag_data!)
                self.select_widgets(self.selected_widgets, update_drag_start=False)
                self._draw_group_highlights()
                self.update_status(f"✨ Выделено: {len(self.selected_widgets)} виджетов")
                return  # ← ВАЖНО: выходим, чтобы не сработал обычный клик ниже

            # 🎯 ОБЫЧНЫЙ КЛИК (без модификаторов)
            if clicked.group_id:
                # Выделяем ВСЮ группу
                group_widgets = [w for w in self.widgets if w.group_id == clicked.group_id]
                self.deselect_all()
                self.selected_widgets = [str(w.id) for w in group_widgets]
                self.select_widgets(self.selected_widgets)
                self._draw_group_highlights()
                self.update_status(f"📦 Выделена группа: {len(group_widgets)} виджетов")
            else:
                # Выделяем один виджет
                self.deselect_all()
                self.selected_widgets = [str(clicked.id)]
                self.select_widgets(self.selected_widgets)
                self.update_status(f"✨ Выбран: {clicked.widget_type}")

            # Сохраняем точку начала перетаскивания
            ...
            # Если НЕ зажаты модификаторы — снимаем выделение
    ...
    def on_canvas_drag(self, event):
        # Marquee selection (выделение прямоугольником)
        if self.drag_data.get("marquee_selecting") and self.drag_data.get("marquee_rect"):
            sx = self.drag_data["marquee_start_x"]
            sy = self.drag_data["marquee_start_y"]
            # Обновляем координаты прямоугольника
            self.canvas.coords(self.drag_data["marquee_rect"], sx, sy, event.x, event.y)
            return
        ...
        # Move (с группировкой) - простая и надёжная реализация
        if self.selected_widgets and not self.drag_data.get("resizing"):
            dx = event.x - self.drag_data["start_x"]
            dy = event.y - self.drag_data["start_y"]

            # Собираем все виджеты для перемещения (включая группы)
            widgets_to_move = []
            for wid in self.selected_widgets:
                wo = next((w for w in self.widgets if str(w.id) == wid), None)
                if wo and wo not in widgets_to_move:
                    widgets_to_move.append(wo)
                    # Если виджет в группе, добавляем всю группу
                    if wo.group_id:
                        for w in self.widgets:
                            if w.group_id == wo.group_id and w not in widgets_to_move:
                                widgets_to_move.append(w)

            # Двигаем все виджеты
            for wo in widgets_to_move:
                if wo.locked:
                    continue
                ...
                actual_dx = new_x - wo.x
                actual_dy = new_y - wo.y

                if wo.widget_instance:
                    self.canvas.move(wo.widget_instance, actual_dx, actual_dy)
                if wo.selection_rect:
                    self.canvas.move(wo.selection_rect, actual_dx, actual_dy)
                for h in wo.resize_handles:
                    self.canvas.move(h, actual_dx, actual_dy)
    ...
    def on_canvas_double_click(self, event):
        # Двойной клик на виджете - выделить всю его группу
        for wo in reversed(self.widgets):
            if not wo.visible: continue
            if (wo.x<=event.x<=wo.x+wo.current_width and
                    wo.y<=event.y<=wo.y+wo.current_height):
                if wo.group_id:
                    # Выделяем всю группу
                    group_widgets = [w for w in self.widgets if w.group_id == wo.group_id]
                    self.deselect_all()
                    self.selected_widgets = [str(w.id) for w in group_widgets]
                    self.select_widgets(self.selected_widgets)
                    self.update_status(f"Выделена группа: {len(group_widgets)} виджетов")
                else:
                    # Если не в группе - просто выделяем этот виджет
                    self.deselect_all()
                    self.selected_widgets = [str(wo.id)]
                    self.select_widgets(self.selected_widgets)
                return

        # Если не кликнули на виджет - добавляем виджет
    ...
    def render_widget(self, wo):
        ...
        try:
            ...
            def make_handler(obj):
                def on_press(e):
                    # Если режим добавления или виджет заблокирован - ничего не делаем
                    ...
                    if obj.group_id:
                        group_members = [w for w in self.widgets if w.group_id == obj.group_id]
                        self.deselect_all()
                        self.selected_widgets = [str(w.id) for w in group_members]
                        self.select_widgets(self.selected_widgets, update_drag_start=False)

                        # Цели для перетаскивания - все участники группы
                        targets = group_members
                    else:
                        # Если не в группе - работаем с одним виджетом
                        if str(obj.id) not in self.selected_widgets:
                            ...
                            # Если уже выделен, тащим текущее выделение
                            if obj.group_id:
                                targets = [w for w in self.widgets if w.group_id == obj.group_id]
                            else:
                                targets = [obj]

                    # Сохраняем начальные позиции для ВСЕХ объектов, которые будем тащить
                    self.drag_data["drag_targets"] = targets
                    self.drag_data["drag_start_mouse"] = (e.x_root, e.y_root)
                    self.drag_data["drag_start_positions"] = {w.id: (w.x, w.y) for w in targets}

                def on_drag(e):
                    if "drag_targets" not in self.drag_data:
                        return "break"  # Прерываем, если нет данных

                    # Вычисляем смещение мыши
                    start_x, start_y = self.drag_data["drag_start_mouse"]
                    dx = e.x_root - start_x
                    dy = e.y_root - start_y

                    # Двигаем ВСЕ объекты из группы (или один, если группы нет)
                    for w in self.drag_data["drag_targets"]:
                        if w.locked: continue

                        # Получаем оригинальную позицию
                        ox, oy = self.drag_data["drag_start_positions"][w.id]
                        ...
                        if w.widget_instance:
                            self.canvas.coords(w.widget_instance, new_x, new_y)

                        if w.selection_rect:
                            self._upd_sel_bounds(w)

                    return "break"  # ВАЖНО: прерываем событие, чтобы canvas не двигал их второй раз

                def on_release(e):
                    if "drag_targets" in self.drag_data:
                        # Чистим данные
                        del self.drag_data["drag_targets"]
                        del self.drag_data["drag_start_positions"]
                        del self.drag_data["drag_start_mouse"]
            ...
            self.update_canvas_scrollregion()
    # ── SELECTION (множественное выделение) ───────────────────────────────────────
    def select_widgets(self, ids, update_drag_start=True):
        """Выделение виджетов — исправлено для мульти-выбора"""

        # 🔥 КЛЮЧЕВОЙ МОМЕНТ: если update_drag_start=False — НЕ очищаем выделение!
        if update_drag_start:
            self.deselect_all()

        if ids:
            # Убираем дубликаты и невалидные ID
            valid_ids = []
            for wid in ids:
                wo = next((w for w in self.widgets if str(w.id) == wid), None)
                if wo and str(wid) not in valid_ids:
                    valid_ids.append(str(wid))

            self.selected_widgets = valid_ids

            # Вычисляем центр для начала перетаскивания (только если нужно)
            if update_drag_start and self.selected_widgets:
                xs = [w.x + w.current_width / 2 for w in self.widgets if str(w.id) in self.selected_widgets]
                ys = [w.y + w.current_height / 2 for w in self.widgets if str(w.id) in self.selected_widgets]
                if xs and ys:
                    self.drag_data["start_x"] = sum(xs) / len(xs)
                    self.drag_data["start_y"] = sum(ys) / len(ys)

            # Рисуем рамки выделения
            for wid in self.selected_widgets:
                wo = next((w for w in self.widgets if str(w.id) == wid), None)
                if wo and wo.visible:
            ...
            # Статус с информацией о группах
            groups = set(w.group_id for wid in self.selected_widgets
                         if (w := next((x for x in self.widgets if str(x.id) == wid), None)) and w.group_id)
            if groups:
                self.update_status(f"✨ Выделено {len(self.selected_widgets)} виджетов, {len(groups)} групп(а)")
            else:
                self.update_status(f"✨ Выделено {len(self.selected_widgets)} виджетов")
    ...
    def _draw_group_highlights(self):
        """Убрана подсветка групп по запросу."""
        # Просто очищаем старые элементы, если они остались, и ничего не рисуем
    ...
    def create_rule_widget(self, index, rule):
        ...
        ttk.Combobox(body, textvariable=event_var, values=["click", "double_click", "enter", "leave", "key_press"],
                     width=15, state="readonly").grid(row=0, column=1, sticky=tk.W, padx=10, pady=5)
        ...
        target_combo = ttk.Combobox(body, textvariable=target_var, width=28)
        ...
        COND_UI = {
            ...
            "text_neq": "Текст/значение НЕ РАВНО",
            "chk_on": "Чекбокс отмечен (✔)",
            "chk_off": "Чекбокс НЕ отмечен (⬜)",
            "a_eq_b": "Поле A == Поле B"
        }
        cur_cond = rule.get("condition", "always")
        ...
        tk.Label(ab_frame, text="ПОЛЕ B: ", bg="white", font=("Arial", 9, "bold")).grid(row=1, column=0, sticky=tk.W)
        compare_b_var = tk.StringVar(value=rule.get("compare_b", ""))
        ...
        def toggle_cond_fields(*args):
            ...
            if sel in [COND_UI["text_eq"], COND_UI["text_neq"]]:
                cmp_frame.grid();
                ab_frame.grid_remove()
            elif sel == COND_UI["a_eq_b"]:
                cmp_frame.grid_remove();
                ab_frame.grid()
            else:
                cmp_frame.grid_remove();
                ab_frame.grid_remove()

        cond_var.trace_add("write", toggle_cond_fields)
        toggle_cond_fields()

        # Сохранение
        def save_rule():
            rev_map = {v: k for k, v in COND_UI.items()}
...
"""
    ...
    def _finalize_marquee_selection(self):
        """Завершает выделение прямоугольником - находит виджеты внутри прямоугольника"""
        if not self.drag_data.get("marquee_rect"):
            return

        # Получаем координаты прямоугольника выделения
        coords = self.canvas.coords(self.drag_data["marquee_rect"])
        if len(coords) != 4:
            return
        ...
        # Минимальный размер прямоугольника (чтобы не выделять при случайном клике)
        if abs(x2 - x1) < 5 and abs(y2 - y1) < 5:
            return

        selected_ids = []
        for wo in self.widgets:
            if not wo.visible or wo.locked:
                continue

            # Проверяем пересечение виджета с прямоугольником выделения
            # Виджет выделяется если хотя бы частично пересекается с прямоугольником
            wx1, wy1 = wo.x, wo.y
            wx2, wy2 = wo.x + wo.current_width, wo.y + wo.current_height

            # Проверка пересечения двух прямоугольников
            if (wx1 < sel_x2 and wx2 > sel_x1 and 
                wy1 < sel_y2 and wy2 > sel_y1):
                selected_ids.append(str(wo.id))

        if selected_ids:
            self.select_widgets(selected_ids)
            self.update_status(f"Выделено {len(selected_ids)} виджетов")
        else:
            self.deselect_all()
    ...
    def update_layer_panel(self):
        ...
        for wo in self.widgets:
            if wo.group_id:
                groups.setdefault(wo.group_id, []).append(wo)

        # Показываем группы вверху
        for gid, group_widgets in groups.items():
            group_frame = tk.Frame(self.layer_inner, bg="#e8f4fd", relief=tk.RIDGE, bd=1)
            group_frame.pack(fill=tk.X, padx=2, pady=2)

            # Заголовок группы
            header = tk.Frame(group_frame, bg="#4a90d9")
            header.pack(fill=tk.X)

            # Определяем текущее состояние видимости группы
            any_visible = any(wo.visible for wo in group_widgets)
            eye_icon = "👁" if any_visible else "🚫"

            tk.Label(header, text=f"📦 Группа ({len(group_widgets)})",
                     bg="#4a90d9", fg="white", font=("Arial", 8, "bold")).pack(side=tk.LEFT, padx=5, pady=2)

            # 🟢 ИСПРАВЛЕННАЯ КНОПКА ГЛАЗА (теперь реально скрывает/показывает группу)
            tk.Button(header, text=eye_icon, bg="#4a90d9", fg="white", relief=tk.FLAT, bd=0,
                     font=("Arial", 8),
                     command=lambda g=group_widgets: self._toggle_group_vis(g)).pack(side=tk.RIGHT, padx=2)

            # Кнопка разгруппировать
            tk.Button(header, text="✕", bg="#4a90d9", fg="white", relief=tk.FLAT, bd=0,
                     font=("Arial", 8),
                     command=lambda g=group_widgets: self._ungroup_widgets(g)).pack(side=tk.RIGHT, padx=2)

            # Виджеты в группе
            for wo in group_widgets:
                self._create_layer_row(group_frame, wo, is_in_group=True)

        # Виджеты без группы (отрисовываем в обратном порядке, как в оригинале)
        for wo in reversed(self.widgets):
            if not wo.group_id:
                self._create_layer_row(self.layer_inner, wo, is_in_group=False)

    def _toggle_group_vis(self, group_widgets):
        """Переключает видимость ВСЕХ виджетов в группе сразу"""
        # Если хотя бы один виден -> скрываем все. Иначе -> показываем все.
        new_state = not any(wo.visible for wo in group_widgets)

        for wo in group_widgets:
            wo.visible = new_state
            if new_state:
                self.render_widget(wo)  # Пересоздаёт виджет на холсте
            else:
                # Безопасно скрываем без удаления данных
                if wo.widget_instance:
                    self.canvas.delete(wo.widget_instance)
                    wo.widget_instance = None
                if wo.selection_rect:
                    self.canvas.delete(wo.selection_rect)
                    wo.selection_rect = None
                for h in wo.resize_handles:
                    self.canvas.delete(h)
                wo.resize_handles = []

        self.update_layer_panel()  # Обновит иконку 👁/🚫 в заголовке
        self.undo_manager.save_state("Видимость группы")
    def _create_layer_row(self, parent, wo, is_in_group=False):
        """Создает строку слоя"""
        issel = str(wo.id) in self.selected_widgets
        rbg = "#ddeeff" if issel else ("white" if wo.visible else "#f5f5f5")
        if is_in_group:
            rbg = "#f0f8ff" if issel else "#fafafa"
        row=tk.Frame(parent,bg=rbg,pady=1)
        ...
        # Иконка группы если в группе
        group_icon = "📎 " if wo.group_id else ""
        name=f"{group_icon}{ICONS.get(wo.widget_type,'')} {wo.widget_type}"
        if wo.config.get("text"):
            name += f' - "{wo.config["text"][:15]}"'
    ...
    def _select_group(self, group_widgets):
        """Выделяет все виджеты группы"""
        self.deselect_all()
        self.selected_widgets = [str(wo.id) for wo in group_widgets]
        self.select_widgets(self.selected_widgets)

    def _ungroup_widgets(self, group_widgets):
        """Разгруппировывает виджеты"""
        for wo in group_widgets:
            wo.group_id = None
        self.update_layer_panel()
    ...
    def _toggle_group_vis(self, group_widgets):
        """Переключает видимость ВСЕЙ группы сразу"""
        any_visible = any(wo.visible for wo in group_widgets)
        new_state = 'hidden' if any_visible else 'normal'
        ...
        for wo in group_widgets:
            ...
            if wo.widget_instance:
                try:
                    self.canvas.itemconfigure(wo.widget_instance, state=new_state)
                except:
                    pass
            if wo.selection_rect:
                try:
                    self.canvas.itemconfigure(wo.selection_rect, state=new_state)
                except:
                    pass
            for h in wo.resize_handles:
                try:
                    self.canvas.itemconfigure(h, state=new_state)
                except:
                    pass
    ...
    def _toggle_vis(self, wo):
        """Переключает видимость одного виджета (без удаления!)"""
        wo.visible = not wo.visible
        state = 'normal' if wo.visible else 'hidden'

        if wo.widget_instance:
            try:
                self.canvas.itemconfigure(wo.widget_instance, state=state)
            except:
                pass

        # Скрываем/показываем рамку выделения и ручки
        if wo.selection_rect:
            try:
                self.canvas.itemconfigure(wo.selection_rect, state=state)
            except:
                pass
        for h in wo.resize_handles:
            try:
        ...
        self.update_layer_panel()
        self.undo_manager.save_state("Видимость")
    ...
    def delete_selected(self):
        ...
        for wid in self.selected_widgets[:]:
            wo = next((w for w in self.widgets if str(w.id) == wid), None)
            if wo:
                # 1. Явно удаляем синие квадраты (ручки) и рамку выделения
                for h in wo.resize_handles:
                    self.canvas.delete(h)
                wo.resize_handles = []
                ...
                # 2. Удаляем сам виджет с холста
                if wo.widget_instance:
                    self.canvas.delete(wo.widget_instance)

                # 3. Только теперь удаляем объект из списка
    ...
    def group_selected(self):
        if len(self.selected_widgets) < 2:
            messagebox.showinfo("Группировка", "Выберите минимум 2 виджета (Shift+клик или Ctrl+клик)"); return
        gid = f"group_{int(time.time())}"
        for wid in self.selected_widgets:
            wo = next((w for w in self.widgets if str(w.id) == wid), None)
            if wo: wo.group_id = gid
        self.undo_manager.save_state("Группировка")
        messagebox.showinfo("Группировка", f"Сгруппировано {len(self.selected_widgets)} виджетов")
    ...
    def ungroup_selected(self):
        ...
        else: messagebox.showinfo("Разгруппировка", f"Разгруппировано {cnt} виджетов")

    def _select_current_group(self):
        """Выделяет группу текущего выделенного виджета"""
        if not self.selected_widgets:
            messagebox.showinfo("Группа", "Сначала выберите виджет")
            return

        wo = next((w for w in self.widgets if str(w.id) == self.selected_widgets[0]), None)
        if not wo or not wo.group_id:
            messagebox.showinfo("Группа", "Этот виджет не в группе")
            return
    ...
    def apply_logic_to_preview(self, preview_widgets, canvas):
        """Применяет правила логики к предпросмотру"""
        log_entries = []
        event_map = {
            "click": "<Button-1>", "double_click": "<Double-Button-1>",
            "enter": "<Enter>", "leave": "<Leave>", "key_press": "<KeyRelease>"
        }
    ...
    def _get_inner_from_preview(self, preview_widgets, widget_id):
        """Безопасно получает внутренний Tk-виджет из контейнера предпросмотра"""
        if widget_id not in preview_widgets: return None
        frame = preview_widgets[widget_id]["widget"]
    ...
    def _evaluate_rule_condition(self, rule, preview_widgets, trigger_data=None):
        ...
        def get_val(w_data):
            if not w_data or "widget" not in w_data: return ""
            w = w_data["widget"]
            for child in w.winfo_children():
                cls = child.winfo_class()
                if cls in ("Entry", "Spinbox", "Combobox"):
                    try:
                        return str(child.get()).strip()
                    except:
                        return ""
                if cls in ("Text", "TkText"):
                    try:
                        return str(child.get("1.0", "end-1c")).strip()
                    except:
                        return ""
                if cls == "Checkbutton":
                    try:
                        return "1" if child.instate(["selected"]) else "0"
                    except:
                        return "0"
                if hasattr(child, "cget"):
                    try:
                        return str(child.cget("text")).strip()
                    except:
                        return ""
            return ""
        ...
        if cond in ("if_equal", "if_not_equal"):
            ...
            va = get_val(preview_widgets.get(aid))
            vb = get_val(preview_widgets.get(bid))
    ...
    # ── ЛОГИКА И ПРЕДПРОСМОТР (ИСПРАВЛЕНО) ──────────────────────────────────

    def find_widget_id_by_name(self, target_name, preview_widgets):
        """Находит ID виджета по имени (формат: 'Button (ID: 12345)')"""
        import re
        match = re.search(r'ID:\s*(\d+)', str(target_name))
        if match:
            return match.group(1)
        return None

    def _preview_inner_widget(self, preview_widgets, widget_id):
        """Возвращает внутренний Tk-виджет из контейнера предпросмотра"""
        if not widget_id or widget_id not in preview_widgets:
            return None
        tw = preview_widgets[widget_id]["widget"]
        for child in tw.winfo_children():
    ...
    def _evaluate_rule_condition(self, rule, preview_widgets, trigger_data=None):
        """Проверяет условие правила (текст, чекбокс, сравнение A/B)"""
        cond = (rule.get("condition") or "always").strip()
        if cond == "always":
            return True
    ...
    def bind_preview_event(self, widget, rule, preview_widgets, log_entries, canvas, trigger_data=None, bind_add=False):
        ...
        event_map = {
            "click": "<Button-1>", "double_click": "<Double-Button-1>",
            "enter": "<Enter>", "leave": "<Leave>", "focus": "<FocusIn>",
            "blur": "<FocusOut>", "key_press": "<KeyRelease>"
        }
        ...
        def handler(e):
            ...
            # 2. Находим целевой виджет
            target_widget_data = preview_widgets.get(target_id)
            target_inner = None
            if target_widget_data:
                for child in target_widget_data["widget"].winfo_children():
                    target_inner = child
                    break

            log_msg = f"⚡ {event} → {action}"
            try:
                if action == "set_text" and target_inner:
                    target_inner.config(text=value)
                elif action == "set_bg" and target_inner:
                    target_inner.config(bg=value)
                    if target_widget_data:
                        target_widget_data["widget"].config(bg=value)
                elif action == "show_hide" and target_widget_data:
                    state = 'hidden' if value.lower() == 'hide' else 'normal'
                    canvas.itemconfig(target_widget_data["canvas_id"], state=state)
                elif action == "enable_disable" and target_inner:
                    target_inner.config(state='disabled' if value.lower() == 'disable' else 'normal')
                elif action == "set_value" and target_inner:
                    try:
                ...
                log_entries.append(f"❌ Ошибка: {ex}")
    ...
    def create_preview_widget(self, parent, wt, cfg, original_wo):
        ...
        if wt in ["Label", "Button", "Checkbutton", "Radiobutton", "Menubutton"]:
            ...
            if wt == "Label":
                ...
                inner = tk.Menubutton(w, text=cfg.get("text", "Menu"), bg=cfg.get("bg", "#e0e0e0"),
                                      fg=cfg.get("fg", "black"))
            inner.pack(fill=tk.BOTH, expand=True)
            return w
            ...
            w = tk.Frame(parent, width=width, height=height)
            w.pack_propagate(False)
            inner = tk.Entry(w, width=cfg.get("width", 12), bg=cfg.get("bg", "white"), fg=cfg.get("fg", "black"))
            if cfg.get("text"): inner.insert(0, cfg["text"])
            inner.pack(fill=tk.BOTH, expand=True)
            return w
            ...
            w = tk.Frame(parent, width=width, height=height)
            w.pack_propagate(False)
            inner = tk.Text(w, width=cfg.get("width", 20), height=cfg.get("height", 3), bg=cfg.get("bg", "white"),
                            fg=cfg.get("fg", "black"))
            inner.pack(fill=tk.BOTH, expand=True)
            return w

        elif wt == "Frame":
            w = tk.Frame(parent, width=width, height=height, bg=cfg.get("bg", "#e8e8e8"), relief=tk.GROOVE, bd=1)
            w.pack_propagate(False)
            return w
    ...
    def preview_with_screens(self):
        """Предпросмотр с экранами и логикой"""
        import re
        if not self.screens:
            messagebox.showinfo("Предпросмотр", "Нет экранов.")
            return
        ...
        tk.Label(nav_frame, text="📱 Экраны: ", bg="#2c3e50", fg="white", font=("Arial", 9, "bold")).pack(side=tk.LEFT,
                                                                                                         padx=10)

        canvas_holder = tk.Frame(upper, bg="white")
        canvas_holder.pack(fill=tk.BOTH, expand=True, padx=8, pady=4)

        cv = tk.Canvas(canvas_holder, bg="white", highlightthickness=1, highlightbackground="#ccc")
        sc = ttk.Scrollbar(canvas_holder, orient="vertical", command=cv.yview)
        cv.configure(yscrollcommand=sc.set)
        sc.pack(side=tk.RIGHT, fill=tk.Y)
        cv.pack(side=tk.LEFT, fill=tk.BOTH, expand=True)
        ...
        def render_preview_screen(screen_name):
            screen_state["name"] = screen_name
            cv.delete("all")
            if screen_name not in self.screens: return

            preview_widgets = {}
            max_x, max_y = 400, 300

            for wo in self.screens[screen_name]:
                if not getattr(wo, "visible", True): continue
                try:
                    w = self.create_preview_widget(cv, wo.widget_type, wo.config, wo)
                    cid = cv.create_window(wo.x, wo.y, window=w, anchor="nw")
                    preview_widgets[str(wo.id)] = {"widget": w, "canvas_id": cid, "x": wo.x, "y": wo.y}
                    max_x = max(max_x, wo.x + wo.current_width + 40)
                    max_y = max(max_y, wo.y + wo.current_height + 40)
                except Exception:
                    pass
        ...
        # Кнопки навигации
        for screen_key in self.screens.keys():
            btn = tk.Button(nav_frame, text=screen_key, bg="#34495e", fg="white",
                            command=lambda k=screen_key: render_preview_screen(k))
            btn.pack(side=tk.LEFT, padx=2)

        tk.Button(nav_frame, text="✕ Закрыть", bg="#e74c3c", fg="white", command=win.destroy).pack(side=tk.RIGHT,
                                                                                                   padx=10)

        first = self.current_screen if self.current_screen in self.screens else next(iter(self.screens.keys()))
        render_preview_screen(first)

    def preview(self):
        """Обычный предпросмотр текущего экрана"""
        if self.screens:
            self.preview_with_screens()
        else:
            # Старый метод предпросмотра одного экрана
            win = tk.Toplevel(self.root)
            win.title("👁 Предпросмотр")
            ...
            for wo in self.widgets:
                ...
                try:
                    w = self.create_preview_widget(cv, wo.widget_type, wo.config, wo)
                    cid = cv.create_window(wo.x, wo.y, window=w, anchor="nw")
                    preview_widgets[str(wo.id)] = {"widget": w, "canvas_id": cid}
                except:
                    pass
    ...
    def generate_code(self):
        ...
        for wo in self.widgets:
            ...
            lines.append(f"        {var}.place(x={int(wo.x)}, y={int(wo.y)})")
            lines.append(" ")
        lines += [" ", "if __name__ == '__main__':", "    root = tk.Tk()", "    app = GeneratedApp(root)",
                  "    root.mainloop()"]
        code = "\n".join(lines)
    ...
    def load_project(self):
        ...
        try:
            ...
            for item in data:
                wo = WidgetItem(item["type"], item["x"], item["y"], item["config"])
                wo.events = item.get("events", {});
                wo.visible = item.get("visible", True)
                wo.locked = item.get("locked", False);
                wo.group_id = item.get("group_id", None)
                self.widgets.append(wo);
                self.render_widget(wo)
            self.draw_grid();
            self.update_layer_panel();
            self._update_minimap()
    ...
    def restore_state(self, state):
        ...
        for item in data:
            wo = WidgetItem(item["type"], item["x"], item["y"], item["config"])
            wo.events = item.get("events", {});
            wo.parent_id = item.get("parent_id", None)
            wo.group_id = item.get("group_id", None);
            wo.visible = item.get("visible", True)
            wo.locked = item.get("locked", False)
            self.widgets.append(wo);
